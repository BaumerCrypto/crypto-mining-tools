...
This fires at **00:01 in the host's system time**, not `America/Regina` time.
...
| Host TZ | `1 0 * * *` fires at | Report you receive |
|---|---|---|
| `UTC` | 00:01 UTC = 18:01 `America/Regina` (prev day) | "Yesterday in `America/Regina`" delivered ~6 PM local |
| `America/Regina` | 00:01 `America/Regina` | "Yesterday in `America/Regina`" delivered ~midnight |
| Anywhere else | Whatever 00:01 means on that host | Same report content, different delivery time |
...
With a UTC host and a 00:01 UTC cron, reports arrive ~18 hours after the day they cover ends.
...
- **Daily summaries** are for trend visibility — hashrate drift, thermal behavior, mode-split patterns, uptime trends. None of that is time-sensitive enough to need live-as-of-fire-time data.
...
And in the "No Data" branch, replace any `${TARGET_DATE}` reference with text describing the rolling window — e.g., `"No mining data found in the last 24 hours."`
